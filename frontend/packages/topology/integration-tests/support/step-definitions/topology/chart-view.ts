...
Then('user sees Topology page with message {string}', (message: string) => {
  cy.get(topologyPO.emptyText).should('have.text', message);
});
...
Then(
  ...
  (resourceType1: string, resourceType2: string) => {
    cy.byTestID(resourceType1).should('be.visible');
    cy.byTestID(resourceType2).should('be.visible');
  },
);
